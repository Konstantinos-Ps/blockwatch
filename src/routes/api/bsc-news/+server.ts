...
export const GET: RequestHandler = async () => {
  const url = "https://cryptocurrency-news2.p.rapidapi.com/v1/bsc";
  const options = {
    method: "GET",
    headers: {
  ...
  try {
    ...
    return json(result); // ✅ cleaner, adds content-type automatically
  } catch (error) {
    return json({ error: "API failed" }, { status: 500 }); // ✅ error handling
  }
};
