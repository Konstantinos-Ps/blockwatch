<script lang="ts">
    ...
    const newsQuery = createQuery({
        queryKey: ["cryptodaily"],
        queryFn: async () => {
            const res = await fetch("/api/cryptodaily");
    ...
    const pageTitle = "CryptoDaily - Blockwatch";
    const pageDescription =
        "Stay updated with the latest crypto news and trends from CryptoDaily";
    const pageKeywords =
